...
def visualize_fretboard(matrix: List[List[int]], active_notes: List[Tuple[int, int]]):
    os.system('cls' if os.name == 'nt' else 'clear')  # Clear the console
    print("Guitar Fretboard Visualizer")
    print("---------------------------")
    for string in range(STRINGS):
        for fret in range(FRETS):
            note = matrix[string][fret]
            ...
            if active:
                print("\033[1m\033[91m♦\033[0m", end="")  # Bright red diamond for active notes
            elif brightness > 50:
                print("•", end="")  # Dot for visible inactive notes
            else:
                print("·", end="")  # Small dot for dim inactive notes
        print()  # New line for each string
    print("---------------------------")
...
def main():
    ...
    while True:
        for chord in progression["chords"]:
            ...
            visualize_fretboard(matrix, chord["notes"])
